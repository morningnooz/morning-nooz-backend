...
llm35 = ChatOpenAI(temperature=0, model="gpt-3.5-turbo-16k")
llm4 = ChatOpenAI(temperature=0, model="gpt-4o")
...
SCORE: 6
    ...
    map_chain = LLMChain(llm=llm4, prompt=map_prompt, verbose=True)
    ...
    Be sure to make the summaries 5 sentences each, and try to include multiple citations for each key point.
    Use this format:
    {{
        "summaries": [
            {{
    ...
    # Run chain
    reduce_chain = LLMChain(llm=llm4, prompt=reduce_prompt, verbose=True)
...
def augment_query(query):
    augment_prompt = PromptTemplate.from_template("""
        You are a helpful semantics expert who can generate semantically similar news queries to the text provided for a keyword-based news search engine, returning a list of 2 different options, delimited by |. 
        ---EXAMPLE--- INPUT: nvidia OUTPUT: nvidia gpu|nvidia chips ---END EXAMPLE--- TEXT: {query}""")
    augment_chain = LLMChain(llm=llm35, prompt=augment_prompt)

    mapped_res = augment_chain.run(query=query)

    return mapped_res

# Function to clean and convert to valid JSON
...
def run_process(query):
    ...
    # run query
    # augment query
    augment_queries = augment_query(query).split("|")
    augment_queries.append(query)

    query_results = [search(q) for q in augment_queries]

    print('queries', augment_queries, 'res', query_results)
    logging.info(f"[BING API QUERY RESULTS] {query_results}")
    summaries = build_summary_chain(query, query_results)

    print('🧠summaries', summaries)
    summaries_res = clean_json_string(summaries)

    print('🚀res', summaries_res)

    return summaries_res

# def main():
#     # Your main script logic goes here
#     print(run_process("elon musk"))
